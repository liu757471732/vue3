<template>
  <el-form ref="ruleFormRef" style="width:400px" :model="form_data" :label-width="form_config.labelWidth"
    :size="form_config.formSize" :label-position="form_config.labelPosition" :status-icon="form_config.statusIcon">
    <template v-for="item in form_config.props">
      ...
      <el-form-item :label="item.label" :prop="item.prop" v-else-if="item.type === 'Select'" :key="item.prop + 1">
        <el-select v-model="form_data[item.prop]" :disabled="item.disabled" :clearable="item.clearable"
          ...
          :placeholder="item.message" :size="item.size">
